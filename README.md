...
```
.
├── confinement_model.py         # ядро: 9-элементная модель Мейстера
├── loop_analyzer.py             # ядро: поиск петель
...
├── ai/                          # DeepSeek + Redis-кэш (sentiment/category/severity/summary)
├── analytics/                   # мост: snapshot → ConfinementModel9 → loops
├── agenda/                      # повестка дня + roadmap
├── metrics/                     # snapshot + forecast + OpenWeatherMap
├── db/                          # asyncpg pool + queries + миграции + seed
├── tasks/                       # in-process scheduler (collection / weather / snapshot)
├── api/                         # FastAPI: /api/city/{slug-or-name}/*
├── dashboard/                   # премиум-UI (vanilla JS, Playfair+Manrope)
├── migrations/                  # PostgreSQL + TimescaleDB (опционально)
└── tests/                       # 54 offline unit-теста
```
...
## AI-обогащение (DeepSeek + Redis-кэш)
...
формируются по AI-категории + знаку sentiment. SHA256(model+system+user)
кэшируется в Redis на 24 ч — экономия ~80% токенов при polling-режиме.

Стоимость: ~$0.003 на вызов `/agenda` через DeepSeek (`deepseek-chat`).

## Исторические метрики + прогноз + петли

Scheduler работает в 3 фоновых петлях внутри FastAPI-процесса:

- `collection_loop` (каждые 30 мин): собирает новости → DeepSeek-обогащение → `news`.
- `weather_loop` (каждый час): OpenWeatherMap → `weather`.
- `snapshot_loop` (каждый час): агрегирует 24ч новостей → строка в `metrics`, сразу
  запускает `ConfinementModel9 + CityLoopAnalyzer` → top-3 петли в `loops`.

`/api/city/{slug}/all_metrics` читает свежее из БД: вектора, 7-дневные тренды,
90-дневный OLS-прогноз, активные петли. При пустой БД — плейсхолдеры.
...
В CI: GitHub Actions прогоняет pytest на каждый push и PR (см. бейдж сверху).
`requirements-ci.txt` — минимальный набор без torch/transformers, прогон ~30 с.

## Статус по ТЗ

- [x] §3.3 скелет: `config/`, `collectors/`, `ai/`, `analytics/`, `agenda/`,
      `metrics/`, `db/`, `tasks/`, `api/`, `dashboard/`, `migrations/`.
...
- [x] §3.1 миграции PostgreSQL (+TimescaleDB опционально).
- [x] §4 мультигород: 6 городов, селектор, slug-роутинг, brand-поля.
- [x] AI-обогащение (DeepSeek + Redis-кэш).
- [x] Persistence + in-process scheduler + живые `/all_metrics`.
- [x] Исторические метрики + тренды 7д + OLS-прогноз 90д.
- [x] Подключение ядра `ConfinementModel9` + `CityLoopAnalyzer`.
- [x] CI (GitHub Actions) — pytest на каждом PR.
- [ ] Полная миграция фронтенда на Next.js + Framer Motion (vanilla MVP
      покрывает требования к визуалу — миграция планируется отдельно).
